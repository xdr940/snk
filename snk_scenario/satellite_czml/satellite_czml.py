...
from .czml import (CZML, Billboard, CZMLPacket, Description, Label,
                   Path, Position, Point,Orientation)
...
class satellite():
    ...
    czmlPath = None
    czmlPosition = None
    czmlOrientation= None

    def __init__(self, tle, name=None, description=None, color=None, image=None,
                 marker_scale=None, use_default_image=True, start_time=None, end_time=None,time_step=None,
                 show_label=True, show_path=True):

        # Validate the inputs
        self.num_id = str(tle[1][2:7])
        self.id = tle[0]
        # self.label_text = self.num_id
        ...
        if name is None:
            ...
            sat_type, self.num_id = name.split('-')
            self.name = name
            if sat_type == 'MS':
    ...
    def build_marker(self,
                     ...
                     size=None,
                     color=None,
                     outlineColor=[255, 255, 255, 255],# white
                     outlineWidth=2,
                     rebuild=False):
        '''
    ...
    def build_label(self,
        ...
        if self.czmlLabel is None or rebuild:
            self.czmlLabel = Label(text=self.label_text, show=show or self.show_label)
            self.czmlLabel.fillColor = {"rgba": color or self.label_color}
    ...
    def build_orientation(self,rebuild=False):
        if self.czmlOrientation is None or rebuild:
            self.czmlOrientation = Orientation()
            self.czmlOrientation.velocityReference = "#position"
        return self.czmlOrientation
    ...
    def build_position(self,
        ...
        if self.czmlPosition is None:
            ...
            number_of_positions = int((end_time - start_time).total_seconds()/self.time_step)
...
class SatelliteCzml():
    ...
    def __init__(self,
                 ...
                 seed=None,
                 ignore_bad_tles=False,
                  orb_num=None,sat_num=None,
                 time_step = None): #addtional
        '''
        ...
        if not color_list and orb_num and sat_num:
            color_list=[]
    ...
    def get_czml(self):
        ...
        for id, sat in tqdm(self.satellites.items()):
            ...
            try:
                sat_packet = CZMLPacket(id=id,name=sat.name)
                ...
                sat_packet.position = sat.build_position(referenceFrame=self.referenceFrame)
                sat_packet.orientation = sat.build_orientation()
    ...
    def get_json(self):
        json_fmt = self.get_czml()
        json_fmt = json.loads(json_fmt)
        return json_fmt[1:]



def get_real_entities(entities,parent):
    ret_entities = []
    for entity in entities:
        if 'parent' in entity.keys() and entity['parent'] == parent:
